...
| 02 | 목차 |
| 03 | 01 비전 및 미션 |
| 04-06 | 02 하드웨어 (DGX 컴퓨트 / GPU·엣지 / 인프라·플랫폼) |
| 07-08 | 03 파트너십 (기술·금융 / 미디어·데이터센터) |
| 09 | 04 마스터 로드맵 개요 |
| 10-14 | 04 Phase 1a·1b·2·3·4 |
| 15 | **05 국내 로드맵 (8단계)** |
| 16-17 | 06 사이드 로드맵 A·B / C |
| 18 | 07 NASDAQ 여정 |
| 19 | 맺음 |
...
## 가독성 기준 (2026-09-16 전면 개편)

고연령 독자가 한눈에 읽히는 것이 이 덱의 설계 기준이다. **글자를 줄이는 방향의 수정은 하지 말 것.**

- **타입 스케일 레버는 `deck.css` 의 `html{font-size:21px}` 한 줄**이다(브라우저 기본 16px 대비 1.31배).
  본문 크기가 전부 `rem` 이라 이 값만 올리면 전체가 커진다. 올린 뒤에는 반드시 넘침을 다시 잴 것.
- 실측 크기(1280x720 기준): 표 20px · 국내 로드맵 20px · 파트너 19.5px · 타임라인 18.5px · NASDAQ 18.5px.
  화면에서는 프레임이 확대되므로 실제로는 더 크게 보인다.
- 본문 대비는 전 구간 **12.5:1 ~ 16:1** 로 WCAG AAA(7:1)의 두 배다. 토큰을 흐리게 되돌리지 말 것.
- 본문 굵기 500~700, 행간 1.42~1.45. 행간을 넓히면 한 장에 안 들어간다.
- **여백 최소화**: 카드 그리드는 `align-content` 를 지정하지 않아 본문 높이를 채운다.
  `center` 로 되돌리면 아래가 빈다. 표는 `table{height:100%}` 로 행이 높이를 나눠 갖는다.
- 검증: 한/영 양쪽에서 `scrollHeight > clientHeight` 인 요소가 0개여야 하고,
  각 장의 `.slide-body` 남는 여백이 40px 를 넘지 않아야 한다.
